(ns opensea.main
  "Kotodama WASM entrypoint for the Opensea clean-room actor (L5) — Clojure port.
  ...
  facts (`opensea.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "opensea")
(def tier "L5")
...
(def entity-specs
  [{:entity "Nft"        :plural "nfts"        :id-prefix "opensea_nft"
    :fields [:identifier :collection :contract :tokenStandard :name :description :imageUrl :openseaUrl :isDisabled :isNsfw :updatedAt] :required [:identifier :collection]
    :coerce {}                       :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "opensea_col"
    :fields [:collection :name :description :imageUrl :safelistStatus :isDisabled :isNsfw :totalSupply :createdDate] :required [:collection :name]
    :coerce {:totalSupply :int}      :refs {}}
   {:entity "Listing"    :plural "listings"    :id-prefix "opensea_lis"
    :fields [:orderHash :chain :protocolAddress :remainingQuantity :orderCreatedAt :type :status] :required [:orderHash :chain]
    :coerce {:remainingQuantity :int :orderCreatedAt :int} :refs {}}
   {:entity "Offer"      :plural "offers"      :id-prefix "opensea_off"
    :fields [:orderHash :chain :protocolAddress :remainingQuantity :orderCreatedAt :status] :required [:orderHash :chain]
    :coerce {:remainingQuantity :int :orderCreatedAt :int} :refs {}}
   {:entity "Account"    :plural "accounts"    :id-prefix "opensea_acc"
    :fields [:address :username :bio :website :joinedDate] :required [:address :username]
    :coerce {}                       :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"opensea.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "opensea-compat" :tier tier :entities entities} 200])
...
(defn start! [] :opensea-compat/ready)
